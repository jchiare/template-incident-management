...
export default {
  blocks: (
    ...
    return [
      section(
        `*CAN Report | <!date^${currentTimeInMS}^{date_short} {time}|${currentTimeInMS}>* | <#${channelID}>`
      ),
      ...
      divider(),
      context(
        `Next update<!date^${nextReportInMS}^ is {date_short_pretty} at {time}| in ${nextReportTime} minutes>`
      )
    ];
  }
};
